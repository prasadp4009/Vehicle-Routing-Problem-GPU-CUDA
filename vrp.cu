 ...
 */
__global__ void
savingsCalc(const float *costMatrix, float *savingsMatrix, int numNodes)
{
...
main(void)
{
    ...
    int threadsPerBlock = 256;
	  int blocksPerGrid = ((numNodes*numNodes) + threadsPerBlock - 1) / threadsPerBlock;
    ...
    for (int i = 0; i < numNodes; ++i)
    {
        for(int j = 0; j < numNodes; ++j){
          printf("%2d/t", savingsMatrix_h[i][j]);
